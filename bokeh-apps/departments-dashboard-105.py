...
source = ColumnDataSource(data=dict(x=[], y=[], dept=[], days_open=[], status=[], case_title=[], source=[], id=[], photo=[]))
...
color_mapper = CategoricalColorMapper(palette=['#377eb8', '#e41a1c'], factors=status_list)


# create circle glyphs with wm_y and wm_x coordinates as x and y
p.circle('x', 'y', source=source, alpha=0.8, color={'field': 'status', 'transform': color_mapper})

# filter the DataFrame based on the Department Title
def select_requests():
    dept_val = dept.value
    filtered_df = service_requests
    filtered_df = filtered_df[filtered_df.Department.str.contains(dept_val)==True]
    return filtered_df
